...
def find_chrelu_gate(x_test,model,acts):
    ...
    for j in range(0, length):
        ...
        for i in range(0, l):
            ...
            if len(num.shape)==4:
                # 说明是卷积层
                for m in range(0,num.shape[0]): # m是batch，就是1。（后期可以根据这一点修改为多张图片一起处理）
                ...
                # 线性层
                for m in range(0,num.shape[0]): # m是batch，就是1。
                    for n in range(0,num.shape[-1]): # n是通道，线性层的通道就是个数
